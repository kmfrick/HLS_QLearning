# 1 "HLS_CartPole/pole.c"
# 1 "HLS_CartPole/pole.c" 1
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 149 "<built-in>" 3
# 1 "<command line>" 1
...
# 1 "/home/kmfrick/.local/opt/Xilinx/Vivado/2019.2/common/technology/autopilot/etc/autopilot_ssdm_op.h" 1
# 305 "/home/kmfrick/.local/opt/Xilinx/Vivado/2019.2/common/technology/autopilot/etc/autopilot_ssdm_op.h"
...
# 7 "<command line>" 2
# 1 "<built-in>" 2
# 1 "HLS_CartPole/pole.c" 2
# 1 "HLS_CartPole/globals.h" 1
# 34 "HLS_CartPole/globals.h"
...
# 1 "/home/kmfrick/.local/opt/Xilinx/Vivado/2019.2/lnx64/tools/clang/bin/../lib/clang/3.1/include/stddef.h" 1 3 4
# 31 "/home/kmfrick/.local/opt/Xilinx/Vivado/2019.2/lnx64/tools/clang/bin/../lib/clang/3.1/include/stddef.h" 3 4
...
# 1 "/home/kmfrick/.local/opt/Xilinx/Vivado/2019.2/lnx64/tools/clang/bin/../lib/clang/3.1/include/stdarg.h" 1 3 4
# 30 "/home/kmfrick/.local/opt/Xilinx/Vivado/2019.2/lnx64/tools/clang/bin/../lib/clang/3.1/include/stdarg.h" 3 4
typedef __builtin_va_list va_list;
# 48 "/home/kmfrick/.local/opt/Xilinx/Vivado/2019.2/lnx64/tools/clang/bin/../lib/clang/3.1/include/stdarg.h" 3 4
...
# 37 "HLS_CartPole/globals.h" 2
# 57 "HLS_CartPole/globals.h"
typedef float vector[162];
...
int learn(int y, vector w,
  ...
  float rhat);
# 85 "HLS_CartPole/globals.h"
void cart_pole(int action, float *x, float *x_dot, float *theta, float *theta_dot);
# 98 "HLS_CartPole/globals.h"
int get_box(float x, float x_dot, float theta, float theta_dot);
# 2 "HLS_CartPole/pole.c" 2

int learn(int y, vector w,
  int box, vector e, vector xbar, float oldp, vector v, float x,
  float x_dot, float theta, float theta_dot, float r, float p,
  float rhat) {_ssdm_SpecArrayDimSize(w, 162);_ssdm_SpecArrayDimSize(e, 162);_ssdm_SpecArrayDimSize(xbar, 162);_ssdm_SpecArrayDimSize(v, 162);
 int failures;
 int i;
 int failed;
 int steps;
 ...
 while (steps++ < 100000 && failures < 100) {

  y = (((float)(rng_state = (a * rng_state + c) % m) / (float)((1 << 31) - 1)) * (rng_state < 0 ? -1 : 1) < (1.0 / (1.0 + pow(2.7182818284590452354, -((-50.0 >= ((w[box] <= 50.0) ? w[box] : 50.0)) ? -50.0 : ((w[box] <= 50.0) ? w[box] : 50.0))))));
...
void cart_pole(int action, float *x, float *x_dot, float *theta, float *theta_dot)
 ...
 force = (action > 0) ? 10.0 : -10.0;
 costheta = cosf(*theta);
 sintheta = sinf(*theta);

 temp = (force + (0.1 * 0.5) * *theta_dot * *theta_dot * sintheta)
   / (0.1 + 1.0);
 ...
 thetaacc =
   (9.8 * sintheta - costheta * temp)
     / (0.5
       * (1.3333333333333
         - 0.1 * costheta * costheta
           / (0.1 + 1.0)));

 xacc = temp - (0.1 * 0.5) * thetaacc * costheta / (0.1 + 1.0);
...
int get_box(float x, float x_dot, float theta, float theta_dot)
 ...
 if (theta < -0.1047192)
  ;
 else if (theta < -0.0174532)
  box += 9;
 else if (theta < 0)
  box += 18;
 else if (theta < 0.0174532)
  box += 27;
 else if (theta < 0.1047192)
  box += 36;
 else
  box += 45;
